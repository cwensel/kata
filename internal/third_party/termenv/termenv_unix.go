...
func (o Output) termStatusReport(sequence int) (string, error) {
	...
	// terminals concurrently. Some nested hosts keep TERM=xterm-* inside tmux
	// while preserving TMUX; probing there leaks the OSC/CPR responses back to
	// the prompt, so treat TMUX itself as authoritative.
	term := o.environ.Getenv("TERM")
	if strings.HasPrefix(term, "screen") || strings.HasPrefix(term, "tmux") ||
		strings.HasPrefix(term, "dumb") || o.environ.Getenv("TMUX") != "" {
