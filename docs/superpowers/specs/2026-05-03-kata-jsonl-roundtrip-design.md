...
- **`purge_log.issue_uid` / `purge_log.project_uid`** = the imported issue/project UIDs looked up by `purged_issue_id` / `project_id`. The issue may already be gone (purge cascade removed it) — in that case the column stays NULL. This is the only legal NULL `purge_log.issue_uid` / `project_uid`: a v1 purge that completed before the cutover. Future purges (v2+) populate these columns from the live `issues.uid` / `projects.uid` read at purge time.
